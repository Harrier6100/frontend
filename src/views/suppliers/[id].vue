<template>
    <h3>{{ id ? t('MENU.SUPPLIERS_ID') : t('MENU.SUPPLIERS_NEW') }}</h3>
    <Form @submit.prevent="onSave">
        <div>
            <Label>{{ t('LABEL.SUPPLIERS.ID') }}</Label>
            <Input v-model="form.id" />
            <Message :error="errors.id" />
        </div>
        <div>
            <Label>{{ t('LABEL.SUPPLIERS.NAME') }}</Label>
            <Input v-model="form.name" />
            <Message :error="errors.name" />
        </div>
        <div>
            <Label>{{ t('LABEL.SUPPLIERS.REMARKS') }}</Label>
...
const schema = yup.object({
    id: yup.string().required().label('LABEL.SUPPLIERS.ID'),
    name: yup.string().required().label('LABEL.SUPPLIERS.NAME'),
});
...
onMounted(async () => {
    ...
    try {
        startLoading();
        await execute(async () => {
            const responseData = await supplierService.get(id);
...
const onSave = async () => {
    ...
    try {
        startLoading();
        await execute(async () => {
            if (id) await supplierService.update(id, form.value);
            else await supplierService.create(form.value);
